...
for i in ['Ele','Compunds','delta_G','Eg','Ehull',
          'Nb','Bh',
          'EN','FIE',
          'MR','AN','MP','BP','SOA']:
    names.remove(i)
...
# df = df[names].sort_values(by=['d'],ascending=False)[:-4]
...
for num_features in range(1,6):
    ...
    results.to_csv('order'+str(num_features)+f'_knn_{repeats}.csv',index=False)
